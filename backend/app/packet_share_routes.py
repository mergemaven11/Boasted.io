...
def _shared_html(packet: dict[str, Any], *, allow_download: bool, token: str, access_code: str | None) -> str:
    ...
    return f"""<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><meta name='robots' content='noindex,nofollow'><meta name='referrer' content='no-referrer'><title>{escape(str(packet.get('title') or 'Boasted Packet'))}</title><style>
