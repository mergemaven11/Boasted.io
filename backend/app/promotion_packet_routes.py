...
def _promotion_summary(packet: dict, target_role: str, target_level: str) -> str:
    ...
    if target:
        parts.append(f"This packet organizes the evidence for a progression conversation toward {target}.")
    else:
        parts.append("This packet organizes the evidence for a promotion or increased-scope conversation.")
    ...
    if scorecard.get("impact_receipts"):
        parts.append(
            ...
            f"{scorecard.get('confirmed_assertions', 0)} confirmed recognition"
...
def _strengthening_actions(scorecard: dict) -> list[dict]:
    ...
    if scorecard.get("quantified_result_coverage_percent", 0) < 60:
        actions.append({"area": "Measurable impact", "action": "Add numbers, counts, percentages, time saved, quality changes, or other measurable outcomes where they are genuinely known.", "why": "Concrete outcomes make scope and impact easier to evaluate without inventing a score."})
...
def _add_recognition(packet: dict) -> None:
    ...
    for receipt in packet.get("receipt_records", []) or []:
        ...
        recognition_by_reference[receipt.get("reference")] = recognition
    for contribution in packet.get("contribution_records", []) or []:
        contribution["recognition"] = normalize_recognition(contribution.get("confirmations"), []) or recognition_by_reference.get(contribution.get("reference"), [])


def _build_promotion_packet(
    *, current_user: dict, start_date, end_date, career_area: str | None,
    ...
    packet = _build_packet(
        ...
        career_area=career_area, role_title=role_title, organization=organization,
        confidential=confidential,
    )["packet"]
    _add_recognition(packet)

    target_role_value = (target_role or "").strip()
    target_level_value = (target_level or "").strip()
    scorecard = packet.get("scorecard", {})
    recognized_records = [record for record in packet.get("contribution_records", []) if record.get("recognition")]

    summary = _promotion_summary(packet, target_role_value, target_level_value)
    packet.update({
        "kind": "promotion",
        "title": "Promotion Packet",
        "target": {"role": target_role_value, "level": target_level_value},
        ...
        "promotion_case": {
            ...
            "verified_recognition": recognized_records,
...
def get_promotion_packet(
    ...
    confidential: bool = Query(True), target_role: str | None = Query(None, max_length=160),
    ...
    return _build_promotion_packet(
        current_user=current_user, start_date=parsed_start, end_date=parsed_end,
        career_area=career_area, role_title=role_title, organization=organization,
        confidential=confidential, target_role=target_role, target_level=target_level,
    )
