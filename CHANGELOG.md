...
### Changed
- Made the globally required Frontend CI check report on every pull request while keeping the full browser/lint/build/bundle suite scoped to frontend-relevant changes, preventing non-frontend PRs from getting stuck behind a required check that never starts. (#199)
