...
function AuthScreen({ onSuccess }) {
  ...
  const contentWidth = Math.max(280, Math.min(width - (width >= 768 ? 64 : 24), 560));
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        <View style={[styles.loginShell, { width: contentWidth }]}> 
          ...
          <View style={[styles.card, styles.loginCard, compact && styles.loginCardCompact]}>
...
function Page({ kicker, title, children, refreshing = false, onRefresh }) {
  const { width } = useWindowDimensions();
  const contentWidth = Math.max(280, Math.min(width - 28, 760));
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView
        contentContainerStyle={styles.pageFrame}
        showsVerticalScrollIndicator={false}
  ...
  return (
    <Page kicker="YOUR CAREER PROOF" title={`Welcome${user?.name ? `, ${user.name.split(' ')[0]}` : ''}.`} refreshing={refreshing} onRefresh={onRefresh}>
      <View style={styles.hero}>
        ...
        <Text style={styles.heroTitle}>Turn fresh wins into durable career proof.</Text>
...
function Proof({ overview, loading, error, refreshing, onRefresh }) {
  return (
    <Page kicker="PROOF LIBRARY" title="Your proof, synced." refreshing={refreshing} onRefresh={onRefresh}>
      ...
      {!loading && overview.receipts.length > 0 ? <Text style={styles.sectionLabel}>IMPACT RECEIPTS</Text> : null}
      ...
      {!loading && overview.entries.length > 0 ? <Text style={styles.sectionLabel}>ACCOMPLISHMENTS</Text> : null}
      {overview.entries.map((entry) => (
        <ProofCard key={`entry-${entry.id}`} title={entry.title} body={entry.impact || entry.action} status={entry.category || 'Accomplishment'} isPublic={entry.is_public} />
      ))}

      {!loading && !error && overview.entries.length === 0 && overview.receipts.length === 0 ? (
...
function Add({ navigation, onCreated }) {
  ...
  return (
    <Page kicker="QUICK CAPTURE" title="Catch the win while it is fresh.">
      <View style={styles.card}>
...
function Profile({ user, onUserChange }) {
  ...
  return (
    <Page kicker="PROFESSIONAL STORY" title="Your story. Your control.">
      <View style={styles.card}>
...
function Settings({ user, onSignOut }) {
  ...
  return (
    <Page kicker="ACCOUNT" title="Built to keep your proof yours.">
...
const styles = StyleSheet.create({
  ...
  loginPageCompact: { justifyContent: 'flex-start', paddingVertical: 18 },
  ...
  tabBar: { backgroundColor: colors.sidebar, borderTopColor: colors.border, height: 76, paddingTop: 7, paddingBottom: 9 },
