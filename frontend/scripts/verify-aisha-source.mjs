...
function runProcess(command, args, timeoutMs = 30000) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => { child.kill("SIGKILL"); reject(new Error(`Timed out after ${timeoutMs}ms: ${command}`)); }, timeoutMs);
...
requestAnimationFrame(()=>requestAnimationFrame(()=>{const stage=document.querySelector(".interview-video-stage");const shell=document.querySelector(".aisha-avatar-shell");const photo=document.querySelector(".aisha-photo-avatar");const sr=stage?.getBoundingClientRect();const ar=shell?.getBoundingClientRect();const ok=Boolean(stage&&shell&&photo&&sr&&ar&&ar.width>=sr.width*.99&&ar.height>=sr.height*.99&&shell.getAttribute("data-avatar-engine")==="bragstack-photo-v2"&&shell.getAttribute("data-aisha-state")==="speaking"&&photo.complete&&photo.naturalWidth>0&&!document.querySelector(".aisha-vector-avatar,.aisha-mouth-opening"));document.body.dataset.aishaResult=ok?"ok":"fail";document.body.dataset.aishaDetails=[Math.round(sr?.width||0),Math.round(sr?.height||0),Math.round(ar?.width||0),Math.round(ar?.height||0),photo?.naturalWidth||0,photo?.naturalHeight||0,shell?.getAttribute("data-aisha-state")].join(":");}));
`);
...
try {
  ...
  assert.match(run.stdout, /data-aisha-details="[0-9]+:[0-9]+:[0-9]+:[0-9]+:[1-9][0-9]*:[1-9][0-9]*:speaking"/, "Aisha browser contract must report stage fill, a loaded photo, and speaking state");
} finally {
  await server.close();
  await Promise.allSettled([fs.unlink(harnessHtml), fs.unlink(harnessJsx)]);
}
console.log("Aisha verified: approved photographic interviewer, responsive stage fill, loaded image, interview state, and no retired vector mouth renderer.");
