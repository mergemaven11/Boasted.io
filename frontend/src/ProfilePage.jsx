...
function ProfilePage() {
  ...
  return <main className="profile-settings">
    <header className="profile-settings-header"><div><a className="profile-public-link" href="/app/settings"><ArrowLeft size={16}/> Back to settings</a><p className="profile-settings-eyebrow">Account</p><h1>Edit profile</h1><p>Everything you save here persists on your Boasted account. Public colors and themes are managed separately under Profile appearance.</p></div><div className="profile-header-actions"><a className="profile-public-link" href="/app/settings/appearance">Public profile appearance</a>{user?.public_slug&&<a className="profile-public-link" href={`/brag/${user.public_slug}`} target="_blank" rel="noreferrer">Preview Public Proof Profile <ExternalLink size={16}/></a>}</div></header>
    <form className="profile-settings-card" onSubmit={handleSubmit}>
      ...
      <section className="appearance-section"><div className="appearance-heading"><div><p className="profile-settings-eyebrow"><MessageSquare size={14}/> Connection</p><h2>Open to Talk</h2><p>Choose the kinds of conversations you welcome. Calendly scheduling is configured once under <strong>Settings → Integrations</strong>.</p></div></div>
        <div className="profile-form-grid">
          ...
          <label className="profile-wide">Short note<textarea name="open_to_talk_note" value={connection.open_to_talk_note} onChange={handleConnectionChange} rows={3} maxLength={240} placeholder="Happy to discuss Boasted, platform engineering, virtual coffee, or mentoring."/></label>
        </div>
