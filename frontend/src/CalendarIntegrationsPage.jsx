...
const PROVIDERS = [
  { id: "google", name: "Google Calendar", monogram: "G", description: "Connect Google Calendar to bring upcoming meetings into your private Boasted workspace." },
...
export default function CalendarIntegrationsPage() {
  ...
  function connectProvider(provider){const providerName=PROVIDERS.find((item)=>item.id===provider)?.name||"calendar";setConnectionMessage(`${providerName} is ready for OAuth wiring. Boasted has not connected or read any calendar data yet.`);}

  async function saveCalendly(){
    setCalendlyError("");setCalendlyMessage("");
    const trimmed=calendlyUrl.trim();
    if(trimmed && !isCalendlyUrl(trimmed)){setCalendlyError("Paste a valid https://calendly.com/... scheduling URL so Boasted can render the calendar.");return;}
