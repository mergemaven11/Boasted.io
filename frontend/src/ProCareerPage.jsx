...
function AnalyticsCareerPage() {
  ...
  if (packetMode) {
    return <main className="pro-career-page">
      <header className="pro-page-header"><div><span>BRAGSTACK PRO · CAREER PACKETS</span><h1>{packetMode === "promotion" ? "Build your promotion case." : packetMode === "interview" ? "Build your interview packet." : packetMode === "certification" ? "Build your certification packet." : "Build your performance review."}</h1><p>Turn recorded accomplishments, evidence, results, and skills into a focused professional artifact.</p></div></header>
  ...
  return <main className="pro-career-page">
    <header className="pro-page-header"><div><span>BRAGSTACK PRO · CAREER ANALYTICS</span><h1>Your career proof, visualized.</h1><p>See what your recorded evidence actually says about your work—without invented scores or vanity metrics.</p></div><button onClick={() => void load()}><RefreshCw size={16} />Refresh</button></header>
    {error && <div className="pro-error">{String(error)}</div>}
