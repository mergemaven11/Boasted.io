...
export default function ResumeBuilderStructuredPage() {
  ...
  const missingCoreSections = importedResume && !loadedSavedId && !importedResume.manual ? ["experience", "skills"].filter((section) => !detectedSections.includes(section)) : [];
  ...
  function startFromScratch() {
    ...
    setProofRoleIndex(0);
    setAddedProofKeys([]);
    setMessage("Started a blank resume. Add only the sections that apply to you; work history is optional.");
  }
  ...
  function openSavedResume(savedResume) {
    ...
    setImportedResume({
      ...
      manual: true,
      sections_found: [...(savedResume.experience?.length ? ["experience"] : []), ...(savedResume.skills?.length ? ["skills"] : []), ...Object.keys(sections).filter((key) => sections[key]?.length)],
      sections,
      parse_warnings: [],
    });
    setResumeDraft(draft);
  ...
  async function handleSave({ master = false } = {}) {
    if (!resumeDraft || !importConfirmed) {
      setMessage("Confirm the resume content before saving a version.");
      return;
    }
    const isTargeted = !master && Boolean(result && targetRole.trim() && jobDescription.trim());
    const title = isTargeted ? `${targetRole} · ${new Date().toLocaleDateString()}` : `Master Resume · ${new Date().toLocaleDateString()}`;
    ...
    try {
      const created = await saveResume({
        title,
        target_role: isTargeted ? targetRole : "",
        job_description: isTargeted ? jobDescription : "",
        summary,
  ...
  return (
    <main className="resume-v2-page">
      <header className="resume-v2-hero">
        <div><span className="resume-v2-badge"><Sparkles size={14} /> BOASTED PRO</span><h1>Resume Builder</h1><p>Import the resume you already use or build one from scratch, then target a job and strengthen only the experience you can truthfully support.</p></div>
      ...
      <section className="resume-v2-grid">
        <aside className="resume-v2-panel resume-v2-left">
          ...
          <div className={`resume-v2-upload ${importedResume ? "ready" : ""}`} role="button" tabIndex="0" onClick={() => fileInputRef.current?.click()} onKeyDown={(event) => event.key === "Enter" && fileInputRef.current?.click()}>
            ...
            <Upload size={21} />
            <div>{importedResume ? <><strong>{importedResume.filename}</strong><span>{loadedSavedId ? "Saved structured version" : importedResume.manual ? "Building from scratch" : `${resumeDraft?.experience?.length || 0} role${resumeDraft?.experience?.length === 1 ? "" : "s"} reconstructed`}</span></> : <><strong>{importing ? "Reading resume…" : "Upload existing resume"}</strong><span>PDF, DOCX, or TXT · up to 5 MB</span></>}</div>
        ...
        <section className="resume-v2-panel resume-v2-center">
          {importedResume && resumeDraft && !importConfirmed ? (
            ...
            <div className="resume-v2-empty"><FileText size={44} /><h2>Start with what you have.</h2><p>Upload an existing resume for reconstruction, or build a clean master resume from scratch with work history, education, projects, and skills.</p></div>
          )}
        </section>
        ...
        <aside className="resume-v2-panel resume-v2-right">
          ...
          {result && <>
            ...
            <div className="resume-v2-actions">
              ...
              <button type="button" onClick={() => handleSave({ master: false })}><Save size={15} /> Save targeted version</button>
