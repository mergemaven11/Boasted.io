...
function PacketHeader({ index, title, eyebrow }) { return <header className="packet-page-header"><div><p>{String(index).padStart(2, "0")} · {eyebrow}</p><h2>{title}</h2></div><div className="packet-page-header-mark">BOASTED</div></header>; }
...
function EvidencePages({ packet, startPage }) {
  ...
  return pages.map((items, pageIndex) => <section key={`evidence-${pageIndex}`} className={`packet-sheet packet-document-page ${themeClass(packet)}`}><PacketHeader index={8} eyebrow="Evidence Index" title={pageIndex ? "Evidence Index · continued" : "Source material behind the claims"} />{packet?.sharing_notice && <p className="packet-document-note">{packet.sharing_notice}</p>}{items.length ? <div className="packet-evidence-table"><div className="packet-evidence-row header"><span>Receipt</span><span>Evidence</span><span>Type</span><span>Reference</span></div>{items.map((item, index) => <div className="packet-evidence-row" key={`${item.receipt_reference}-${item.title}-${index}`}><span>{item.receipt_reference}</span><span><strong>{item.title}</strong><small>{item.description}</small></span><span>{formatLabel(item.type)}</span><span>{item.reference || "Stored in Boasted"}</span></div>)}</div> : <EmptyState>No evidence references are included in this packet.</EmptyState>}<PacketFooter page={startPage + pageIndex} /></section>);
}
