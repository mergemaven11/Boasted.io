...
const CONTACT_EMAILS = [
  ...
  },
  {
    ...
    email: "support@boasted.io",
    description: "Sign-in problems, unexpected product behavior, exports, account questions, and help using Boasted.",
  },
  {
    title: "Privacy",
    email: "privacy@boasted.io",
...
const HOW_IT_WORKS = [
  {
    ...
    bullets: [
      ...
      "Reference useful evidence without copying restricted employer or client material into Boasted.",
    ],
  },
  {
    number: "02",
  ...
  },
  {
    ...
    text: "Boasted is designed around private-by-default career evidence. Public proof should be a deliberate subset of your workspace, not a mirror of everything you have recorded.",
    bullets: [
      ...
      "Remember that someone who can view public content may copy or retain it outside Boasted.",
    ],
  },
  {
    number: "05",
...
const USE_CASES = [
  ...
  },
  {
    ...
    use: "Material for internships, scholarships, applications, portfolios, interviews, and early-career storytelling. Boasted accounts are 18+ for now.",
  },
];
...
function Header() {
  return (
    <header className="public-info-topbar">
      <a className="public-info-brand" href="/"><img src="/brandmark.svg" alt="" /><strong>Boasted</strong></a>
  ...
  return (
    <footer className="public-info-footer">
      <div className="public-info-footer-grid">
        <div className="public-info-footer-brand"><a href="/">Boasted</a><p>Turn everyday work into career proof you can use when it matters.</p></div>
      ...
      <div className="public-info-footer-bottom"><span>© 2026 Boasted</span><span>Private by default · Your proof stays yours.</span><div><a href="/login">Log in</a><a href="/register">Start free</a></div></div>
    </footer>
  );
}
  ...
  return (
    <section className="public-info-hero">
      ...
      <aside className="public-info-principles">
        ...
        <span>Boasted is designed to help you organize what you can support, keep private work private, and choose what becomes public.</span>
      </aside>
    </section>
  );
}
...
function HowItWorksPage() {
  return (
    <>
      <Header />
      <Hero
        eyebrow="HOW BOASTED WORKS"
        title="Capture → Prove → Package → Share → Keep building."
        description="Boasted gives your accomplishments somewhere durable to live before the details disappear. Capture the work once, connect the evidence you are allowed to keep, and reuse the strongest proof when a career moment needs it."
        secondaryHref="/nda-safety"
        secondaryLabel="Confidential-work guide"
      />
      <section className="public-info-intro-grid">
        <article><Target size={23} /><h2>What Boasted is</h2><p>A private career-evidence workspace for documenting accomplishments, impact, skills, evidence references, and reusable professional stories.</p></article>
  ...
  return (
    <>
      ...
      <section className="public-info-section public-info-boundaries">
        <div className="public-info-section-heading"><span>BOUNDARIES</span><h2>What Boasted should not become.</h2></div>
  ...
  return (
    <>
      ...
      <Hero
        ...
        primaryHref="mailto:contact@boasted.io?subject=Boasted%20question"
        primaryLabel="Email Boasted"
        secondaryHref="/docs"
      ...
      <section className="public-info-section">
        ...
        <div className="public-info-contact-grid">
          {CONTACT_EMAILS.map((item) => (
            <article key={item.email}><Mail size={21} /><h3>{item.title}</h3><p>{item.description}</p><a href={`mailto:${item.email}?subject=Boasted%20${encodeURIComponent(item.title)}`}>{item.email}</a></article>
          ))}
        </div>
      </section>
      <section className="public-info-section">
...
function TeamPage() {
  return (
    <>
      <Header />
      <Hero
        eyebrow="BOASTED FOR TEAMS"
        badge="COMING SOON"
        title="Support professional growth without turning career evidence into surveillance."
        description="Boasted Team is planned as an organization layer around the employee-controlled evidence model. The goal is to make reviews, recognition, and development easier while keeping private individual proof separate from what a person intentionally shares with an organization."
        primaryHref="mailto:contact@boasted.io?subject=Boasted%20Team%20waitlist"
      ...
      <section className="public-info-section">
        ...
        <div className="public-info-use-grid public-info-use-grid-small">
          <article><Users size={22} /><h3>Shared review templates</h3><p>Give teams a consistent structure for review cycles without forcing every person's private Boasted workspace into the organization.</p></article>
      ...
      <section className="public-info-warning"><Users size={21} /><div><h2>Current availability</h2><p>Individual Boasted accounts are the live product today. Team features are still being developed. Joining the waitlist is an expression of interest, not a purchase commitment or guarantee of a launch date.</p><a href="mailto:contact@boasted.io?subject=Boasted%20Team%20waitlist">Join the Team waitlist →</a></div></section>
      <Footer />
    </>
  );
}
...
function EnterprisePage() {
  return (
    <>
      <Header />
      <Hero
        eyebrow="BOASTED ENTERPRISE"
        badge="EARLY / PLANNED"
        title="Governed career evidence for larger organizations—without pretending the enterprise layer is finished."
        description="The enterprise direction builds on Boasted's employee-controlled evidence model with additional identity, policy, retention, governance, and support capabilities. Enterprise is not presented here as a generally available compliance product today."
        primaryHref="mailto:contact@boasted.io?subject=Boasted%20Enterprise"
        primaryLabel="Discuss enterprise needs"
        secondaryHref="/security"
        secondaryLabel="Security overview"
      />
      <section className="public-info-section">
        <div className="public-info-section-heading"><span>ENTERPRISE ROADMAP</span><h2>Capabilities being considered for larger deployments.</h2><p>The exact package should be driven by real customer requirements, security review, and the controls Boasted can actually support at launch.</p></div>
        <div className="public-info-use-grid public-info-use-grid-small">
      ...
      <section className="public-info-section">
        ...
        <div className="public-info-intro-grid">
          <article><h3>What data does Boasted store?</h3><p>Enterprise evaluation should document the exact data flows, roles, retention behavior, public/private sharing boundaries, and third-party services used by the version being evaluated.</p></article>
          <article><h3>What security attestations exist?</h3><p>Do not assume Boasted holds SOC 2, ISO 27001, HIPAA, FedRAMP, or another certification unless Boasted explicitly publishes that current status. Security claims should match evidence.</p></article>
      ...
      <section className="public-info-warning"><ShieldCheck size={21} /><div><h2>Enterprise is a conversation before it is a contract.</h2><p>Boasted should only commit to controls, integrations, support levels, or compliance requirements after confirming that the product and operating processes can meet them. If your organization has specific procurement, security, privacy, or data-residency requirements, include them in your inquiry.</p><a href="mailto:contact@boasted.io?subject=Boasted%20Enterprise">Start an enterprise conversation →</a></div></section>
      <Footer />
    </>
  );
}
