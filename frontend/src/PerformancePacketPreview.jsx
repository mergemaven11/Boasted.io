...
import "./PacketPlatformPreview.css";
...
function formatPeriod(period = {}) {
  if (period.start_date && period.end_date) return `${formatDate(`${period.start_date}T12:00:00`)} — ${formatDate(`${period.end_date}T12:00:00`)}`;
  return "All recorded work";
}
function CoverageRow({ label, value, note }) {
  const safeValue = Math.max(0, Math.min(100, Number(value) || 0));
  return <div className="packet-coverage-row"><div className="packet-coverage-copy"><div><strong>{label}</strong><span>{note}</span></div><b>{safeValue}%</b></div><div className="packet-coverage-track" aria-hidden="true"><span style={{ width: `${safeValue}%` }} /></div></div>;
}
