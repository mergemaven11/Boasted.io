...
test("new Education accomplishments expose only current 18+ student contexts", () => {
  ...
  assert.match(source, /Career & Education Evidence Library · 18\+/);
});

test("student accomplishment capture is designed for future applications", () => {
  const source = read("./AccomplishmentsPage.jsx");
  assert.match(source, /BragStack Education currently supports users age 18\+/i);
