...
function SeoLandingPage({ content }) {
  ...
  return <main className={`landing-page seo-product-page seo-product-${pageVariant}`}>
    ...
    <section className="seo-product-hero">
      <div className="landing-hero-copy"><div className="landing-eyebrow"><Icon size={16}/>{content.eyebrow}</div><h1>{content.title}</h1><p className="landing-hero-description">{content.description}</p><div className="landing-hero-actions"><a className="landing-btn" href="/register">Start free <ArrowRight size={18}/></a><a className="landing-btn landing-btn-secondary" href="/how-it-works">See how it works</a></div><div className="seo-trust-row" aria-label="Boasted benefits"><span><Check size={15}/> No credit card</span><span><Check size={15}/> Your proof stays connected</span><span><Check size={15}/> Built for real career moments</span></div></div>
    ...
    <section className="seo-explore-section"><div className="seo-explore-heading"><div><p className="landing-mini-label">EXPLORE BOASTED</p><h2>One career story. Connected tools.</h2></div><p>Move between the tools without losing the evidence behind your work.</p></div><nav className="seo-related-grid" aria-label="Related Boasted pages">{relatedLinks.map(({label,href})=><a className="seo-related-card" href={href} key={href}><span>{label}</span><ArrowRight size={18}/></a>)}</nav></section>
