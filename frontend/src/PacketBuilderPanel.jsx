  ...
  return <>
    ...
    <section className="packet-builder-pro" aria-labelledby="packet-builder-title">
      <div className="packet-builder-pro-header">
        ...
        <div><span>BragStack Pro · Professional Packet</span><h2 id="packet-builder-title">Build {spec.label}</h2><p>{spec.description}</p><p className="packet-builder-example"><strong>Example:</strong> {spec.example}</p></div>
      </div>

      <aside className="packet-data-strength-note">
        <Sparkles size={18} />
        <div><strong>More proof makes a stronger packet.</strong><p>The more data you submit—accomplishments, Impact Receipts, evidence, skills, verified recognition, and measurable results—the more accurate and helpful your packet can be. BragStack only uses the proof you have actually saved.</p></div>
      </aside>
      ...
      <div className="packet-builder-pro-footer">
        ...
        <div className="packet-builder-pro-action"><div><BriefcaseBusiness size={16} />Uses your saved BragStack proof</div><button type="button" onClick={onBuild} disabled={isLoading || (isInterview && selectedEntryIds.length === 0)}>{isLoading ? "Building packet…" : `Build ${spec.shortLabel} preview`}</button></div>
      </div>
