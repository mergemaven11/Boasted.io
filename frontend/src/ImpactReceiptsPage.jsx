      ...
      {items.map((item, index) => (
        <div className="receipt-evidence-item" key={`evidence-${index}`}>
          ...
          <div className="receipt-signal-row">
            <label>
              <input type="checkbox" checked={item.is_public} onChange={(event) => onChange(index, "is_public", event.target.checked)} />
              This evidence may be shared publicly
            </label>
            {items.length > 1 && (
              <button type="button" onClick={() => onRemove(index)}>
                <Trash2 size={14} /> Remove evidence
              </button>
            )}
          </div>
        </div>
...
function ImpactReceiptsPage() {
  ...
  async function submitReceipt(event) {
    ...
    const metrics = form.metricLabel.trim() && form.metricValue.trim()
      ...
      : [];
    const evidence = normalizeEvidence(form.evidence).map((item) => ({
      ...item,
  ...
  function addEditEvidence() {
    setEditForm((current) => ({ ...current, evidence: [...current.evidence, { ...EMPTY_EVIDENCE }] }));
  }
  ...
  async function saveEdit(receiptId) {
    ...
    try {
      await updateImpactReceipt(receiptId, {
        ...
        evidence: normalizeEvidence(editForm.evidence).map((item) => ({
          ...item,
          title: item.title.trim(),
          reference: item.reference.trim() || null,
          description: item.description.trim() || null,
      ...
      setEditingId(null);
      setEditForm(null);
      setSuccess("Impact Receipt updated.");
      await loadReceipts();
    } catch (err) {
      setError(err.response?.data?.detail ?? "Impact Receipt could not be updated.");
    } finally {
      setUpdatingId(null);
    }
  }
  ...
  async function toggleVisibility(receipt) {
    ...
    setError("");
    try {
      await updateImpactReceipt(receipt.id, { is_public: !receipt.is_public });
      await loadReceipts();
    } catch (err) {
      setError(err.response?.data?.detail ?? "Receipt visibility could not be changed.");
    } finally {
      setUpdatingId(null);
    }
  }
    ...
    <main className="product-page receipt-library-page">
      <section className="product-page-hero">
        <div>
          ...
          <p>Capture what you accomplished, what changed, the evidence behind it, and the skills you demonstrated. Receipts and evidence stay private unless you explicitly choose to share them.</p>
        </div>
        <button type="button" className="product-page-link" onClick={() => setShowCreate((current) => !current)}>
      ...
      {showCreate && (
        <section className="receipt-library-card receipt-create-card">
          <div className="receipt-library-card-top">
            <div><p className="mini-label">Minimum evidence workflow</p><h2>Create an Impact Receipt</h2></div>
          ...
          <form onSubmit={submitReceipt} className="receipt-create-form">
            ...
            <div className="receipt-proof-columns">
              <label>Metric<input value={form.metricLabel} onChange={(e) => updateForm("metricLabel", e.target.value)} placeholder="e.g. Repeat incidents reduced" /></label>
              <label>Metric value<input value={form.metricValue} onChange={(e) => updateForm("metricValue", e.target.value)} placeholder="e.g. 25%" /></label>
            </div>
      ...
      <section className="receipt-kpi-grid">
        ...
        <article><span className="receipt-kpi-dot" /><span>Confirmed signals</span><strong>{stats.confirmedCount}</strong></article>
      </section>
      ...
      {isLoading ? (
        ...
        <section className="product-empty">
          ...
          <p>Create your first evidence-backed receipt, or turn an existing accomplishment into one.</p>
          <a href="/app/accomplishments">Create from an accomplishment <ExternalLink size={16} /></a>
        </section>
      ) : (
            ...
            <article className="receipt-library-card" key={receipt.id}>
              <div className="receipt-library-card-top">
                ...
                <div className="receipt-card-actions">
                  <button type="button" className={`visibility-pill ${receipt.is_public ? "public" : "private"}`} disabled={updatingId === receipt.id} onClick={() => toggleVisibility(receipt)}>
                    {updatingId === receipt.id ? "Saving…" : receipt.is_public ? "Public" : "Private"}
                  </button>
              ...
              {editingId === receipt.id && editForm ? (
                <div className="receipt-create-form receipt-edit-form">
                ...
                <>
                  <div className="receipt-proof-columns">
                    <div><span>Contribution</span><p>{receipt.contribution}</p></div>
                    <div><span>Result</span><p>{receipt.result}</p></div>
                  </div>
                    ...
                    <div className="receipt-proof-columns">
                      {receipt.metrics.map((metric, index) => (
                        <div key={`${receipt.id}-metric-${index}`}>
                          ...
                          <p><strong>{metric.value}</strong>{metric.context ? ` — ${metric.context}` : ""}</p>
                        </div>
                      ))}
                    </div>
                  )}
                  {receipt.evidence?.length > 0 && (
                    <div className="receipt-evidence-list">
                      ...
                      {receipt.evidence.map((item, index) => (
                        <div className="receipt-evidence-item" key={`${receipt.id}-evidence-${index}`}>
                          <div className="receipt-library-card-top">
                            <strong>{item.title}</strong>
                            <span className={`visibility-pill ${item.is_public ? "public" : "private"}`}>{item.is_public ? "Shareable" : "Private"}</span>
                          </div>
                  ...
                  {receipt.trust_signals?.length > 0 && (
                    <div className="receipt-trust-row">
                      {receipt.trust_signals.map((signal) => <span key={`${receipt.id}-${signal}`}><ShieldCheck size={14} /> {formatLabel(signal)}</span>)}
                    </div>
                  )}
                </>
              )}
